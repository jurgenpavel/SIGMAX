...
function sturges(n: number) {
  return Math.max(4, Math.ceil(1 + Math.log2(Math.max(1, n))));
}
...
export default function Page() {
  ...
  const stats = useMemo(() => {
    ...
    const cp =
      Number.isFinite(usl) && Number.isFinite(lsl) && sig > 0
        ? (usl - lsl) / (6 * sig)
        : NaN;
    ...
    const cpm =
      ...
      Number.isFinite(lsl) &&
      sig > 0 &&
      Number.isFinite(Tused)
        ? (usl - lsl) /
          (6 * Math.sqrt(sig ** 2 + (mu - Tused) ** 2))
        : NaN;

    const Cr = Number.isFinite(cp) && cp > 0 ? 1 / cp : NaN;
    const specHalf =
      Number.isFinite(usl) && Number.isFinite(lsl) ? (usl - lsl) / 2 : NaN;
    const K =
      Number.isFinite(specHalf) &&
      specHalf > 0 &&
      Number.isFinite(mu) &&
      Number.isFinite(Tused)
    ...
    const ppmTwo =
      Number.isFinite(ZUSL) && Number.isFinite(ZLSL)
        ? (1 - phi(ZUSL) + 1 - phi(ZLSL)) * 1_000_000
        : NaN;
    const yieldPct = Number.isFinite(ppmTwo)
      ? (1 - ppmTwo / 1_000_000) * 100
      : NaN;
    const sigmaLevel = Zmin;
    const sigmaShifted = Number.isFinite(Zmin) ? Zmin + 1.5 : NaN;

    const Pp =
      Number.isFinite(usl) && Number.isFinite(lsl) && sigGlobal > 0
        ? (usl - lsl) / (6 * sigGlobal)
        : NaN;
    const Ppi =
      Number.isFinite(lsl) && sigGlobal > 0
        ? (mu - lsl) / (3 * sigGlobal)
        : NaN;
    const Pps =
      Number.isFinite(usl) && sigGlobal > 0
        ? (usl - mu) / (3 * sigGlobal)
        : NaN;
    const Ppk = Math.min(Ppi, Pps);

    const oos = values.filter(
      (v) =>
        (Number.isFinite(lsl) && v < lsl) ||
        (Number.isFinite(usl) && v > usl)
    ).length;
  ...
  function cpDecision(cp: number) {
    ...
    if (cp > 2)
      return { cls: "Clase mundial", decision: "Tiene calidad seis sigma" };
    if (cp >= 1.33 && cp <= 2)
      return { cls: "1", decision: "Más que adecuado" };
    if (cp >= 1 && cp < 1.33)
      return { cls: "2", decision: "Adecuado con control estricto" };
    if (cp >= 0.67 && cp < 1)
      return {
        cls: "3",
        decision: "No adecuado; requiere modificaciones serias",
      };
    return {
      cls: "4",
      decision: "No adecuado; requiere modificaciones serias",
  ...
  const kpiList: { label: string; value: number | string }[] = [
    ...
    {
      label: "PPM (2 lados)",
      value: Number.isFinite(stats.ppmTwo)
        ? Math.round(Number(stats.ppmTwo))
        : "—",
    },
    {
      label: "Yield %",
      value: Number.isFinite(stats.yieldPct)
        ? `${round(Number(stats.yieldPct), 2)}%`
        : "—",
    },
    ...
    <div className="max-w-7xl mx-auto p-6 space-y-6">
      ...
      <div className="rounded-2xl border p-4">
        ...
        <p className="text-sm text-gray-600 mt-3">
          Reglas: Cp &gt; 2 (clase mundial); 1.33 ≤ Cp ≤ 2 (Clase 1);
          1 ≤ Cp &lt; 1.33 (Clase 2); 0.67 ≤ Cp &lt; 1 (Clase 3);
          Cp &lt; 0.67 (Clase 4).
        </p>
      </div>
